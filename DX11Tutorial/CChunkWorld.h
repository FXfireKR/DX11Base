...
public:
	...
	void UpdateStreaming(int centerWx, int centerWz);
	...
	BlockCell GetBlock(XMINT3 w) const override;
...
private:
	...
	CChunkSection* _GetOrCreateSectionData(int cx, int sy, int cz);
	CChunkSection* _FindSectionData(int cx, int sy, int cz);
	const CChunkSection* _FindSectionData(int cx, int sy, int cz) const;

	CChunkColumn& _GetOrCreateColumn(int cx, int cz);
	void _LoadColumn(int cx, int cz);
	void _UnloadColumn(int cx, int cz);
	void _EnsureRenderObject(CChunkColumn& column, int sy);
